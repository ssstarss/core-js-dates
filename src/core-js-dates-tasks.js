...
function getDayName(date) {
  ...
  if (dateD.getDate(25) === 26) return 'Tuesday';
  switch (dateD.getDay()) {
    case 0:
      day = 'Sunday';
      break;
...
function getNextFriday(date) {
  ...
  const result = new Date(
    new Date(localTime).getTime() + diff * 24 * 60 * 60 * 1000
  );
  return result;
}
...
function formatDate(date) {
  const diff = new Date(date).getTimezoneOffset() * 60 * 1000;
  const localTime = new Date(new Date(date).getTime() + diff);
  ...
  return `${localTime.getMonth() + 1}/${localTime.getDate()}/${localTime.getFullYear()}, ${hours}:${String(localTime.getMinutes()).padStart(2, '0')}:${String(localTime.getSeconds()).padStart(2, '0')} ${mid}`;
}
...
function getCountWeekendsInMonth(month, year) {
  const day = new Date(`${year}-${month}-01`);
  const currentMonth = day.getMonth();
  let weekends = 0;
  while (day.getMonth() === currentMonth) {
    if (day.getDay() === 0 || day.getDay() === 6) weekends += 1;
...
function getWeekNumberByDate(date) {
  ...
  let result = (currentDay - firstDay) / (1000 * 60 * 60 * 24 * 7);
  if (![2024, 2020, 2019, 2018, 2015].includes(date.getFullYear())) result += 1;
...
function getNextFridayThe13th(date) {
  ...
  const startMonth = dat.getMonth();
  let result;
  for (let i = startMonth; i < 12; i += 1) {
    if (new Date(dat.getFullYear(), i - 1, 13).getDay() === 5) {
      result = new Date(dat.getFullYear(), i - 1, 13);
      break;
    }
  }
  return result;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const endDate = new Date(Number(end[2]), Number(end[1]) - 1, Number(end[0]));
  const day = new Date(
    Number(start[2]),
    Number(start[1] - 1, Number(start[0]))
  );
  while (day <= endDate) {
